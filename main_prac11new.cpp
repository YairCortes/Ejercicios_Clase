/*Semestre 2019 - 2
	Nombre: Cortés Benitez Yair
	Grupo: 02
	Visual Studio 2017
	Practica 11
		ANIMACION DE UN "MONITO" DE EL PIE DERECHO, PIE IZQUIERDO,
		BRAZO DERECHO E IZQUIERDO Y DE LA CABEZA
		SIMULANDO UN BAILE.
		TECLAS PARA ANIMAR:
		M/m : Brazo izq
		V/v : Brazo der
...
float posX = 0, posY = 2.5, posZ = -3.5, rotRodIzq = 0, rotRodder = 0, brazoIzq = 0, brazoDer = 0, cabeza = 0;
float giroMonito = 0;

#define MAX_FRAMES 30
int i_max_steps = 90;//Si se quiere la animacion mas lento se aumenta
int i_curr_steps = 0;//Inicializa en 0
...
void InitGL ( GLvoid )     // Inicializamos parametros
{
	...
	for(int i=0; i<MAX_FRAMES; i++)
	{
		...
		KeyFrame[i].brazoIzqInc = 0;
		KeyFrame[i].brazoDer = 0;
		KeyFrame[i].brazoDerInc = 0;
		KeyFrame[i].cabeza = 0;
		KeyFrame[i].cabezaInc = 0;
...
void animacion()
{
	...
	{		
		...
		else
		{
			...
			brazoIzq+= KeyFrame[playIndex].brazoIzqInc;
			brazoDer += KeyFrame[playIndex].brazoDerInc;
			cabeza += KeyFrame[playIndex].cabezaInc;
